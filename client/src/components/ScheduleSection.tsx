      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="text-center mb-10 sm:mb-12 md:mb-16 px-4">
          <div className="inline-flex items-center gap-2 px-3 sm:px-4 py-1.5 sm:py-2 bg-primary/10 border border-primary/30 rounded-full mb-4 sm:mb-6">
            <Calendar className="w-3 h-3 sm:w-4 sm:h-4 text-primary animate-pulse" />
            <span className="text-xs sm:text-sm font-mono text-primary uppercase tracking-wider">48 Hours</span>
          </div>

          <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-7xl font-display font-black mb-4 sm:mb-6 bg-gradient-to-r from-orange-500 via-primary to-orange-500 bg-clip-text text-transparent drop-shadow-[0_0_30px_rgba(255,94,0,0.3)]" data-testid="text-schedule-title">
            Event Schedule
          </h2>
        ...
        <div className="max-w-5xl mx-auto space-y-3 sm:space-y-4 md:space-y-6 px-2 sm:px-4">
          {schedule.map((event, index) => (
            <Card
              key={index}
              className={`group relative p-4 sm:p-6 md:p-8 transition-all duration-500 hover:scale-[1.02] overflow-hidden ${
                event.highlight
            ...
            >
              {event.highlight && (
                <>
                  ...
                  <Zap className="absolute top-3 right-3 sm:top-4 sm:right-4 w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6 text-primary animate-pulse" />
                </>
              )}

              <div className="flex flex-col sm:flex-row gap-4 sm:gap-6 sm:items-center relative">
                <div className="flex items-center gap-3 sm:gap-4 sm:w-48 md:w-64">
                  <div className={`w-12 h-12 sm:w-14 sm:h-14 md:w-16 md:h-16 rounded-xl sm:rounded-2xl flex items-center justify-center flex-shrink-0 ${
                    event.highlight 
                    ...
                    <Clock className={`w-5 h-5 sm:w-6 sm:h-6 md:w-7 md:h-7 ${event.highlight ? 'text-white' : 'text-primary'}`} />
                  </div>
                  <div>
                    <p className="font-mono text-xs sm:text-sm text-muted-foreground font-bold mb-0.5 sm:mb-1">{event.day}</p>
                    <p className={`text-base sm:text-lg md:text-xl font-black ${event.highlight ? 'text-primary' : 'text-foreground'}`}>{event.time}</p>
                  </div>
                </div>

                <div className="flex-1">
                  <h3 className={`text-base sm:text-lg md:text-xl lg:text-2xl font-black mb-1 sm:mb-2 ${
                    event.highlight 
                  ...
                  <p className="text-xs sm:text-sm md:text-base lg:text-lg text-muted-foreground mb-2 sm:mb-3">{event.description}</p>
                  <div className="flex items-center gap-1.5 sm:gap-2 text-xs sm:text-sm font-medium">
                    <MapPin className={`w-3 h-3 sm:w-4 sm:h-4 ${event.highlight ? 'text-primary' : 'text-muted-foreground'}`} />
