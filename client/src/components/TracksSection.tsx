        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full max-w-6xl mx-auto">
          <TabsList className="grid w-full grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 bg-transparent p-0 h-auto mb-12" data-testid="tabs-tracks">
            {tracks.map((track) => (
              <TabsTrigger
                key={track.id}
                value={track.id}
                className={`flex flex-col items-center gap-3 p-6 border-2 transition-all duration-300 ${
                  activeTab === track.id
              ...
              >
                ...
                <span className="font-bold text-sm tracking-wide">{track.name}</span>
              </TabsTrigger>
            ))}
          </TabsList>
          ...
          {tracks.map((track) => (
            <TabsContent key={track.id} value={track.id} className="mt-8">
              <Card className="p-10 bg-card/80 backdrop-blur-sm border-2 border-border">
                <div className="flex items-center gap-4 mb-8 pb-6 border-b border-border">
                  <div className="w-16 h-16 border-2 border-foreground/30 flex items-center justify-center">
                    <track.icon className="w-8 h-8 text-foreground" />
                  </div>
                  <div>
                    <h3 className="text-3xl font-bold text-foreground">{track.name}</h3>
                    <p className="text-foreground/60 font-light">Problem Statements</p>
                  </div>
                </div>
                ...
                <div className="space-y-6">
                  {track.problems.map((problem, index) => (
                    <div
                      key={index}
                      className="flex gap-4 p-6 border border-border hover:border-foreground/40 transition-all duration-300"
                      data-testid={`problem-${track.id}-${index}`}
                    >
                      <div className="flex-shrink-0 w-10 h-10 border-2 border-primary/50 flex items-center justify-center text-primary font-bold text-lg">
                        {index + 1}
                      </div>
